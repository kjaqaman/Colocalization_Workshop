...
classdef MatBasics
    methods(Static)
        ...
        function order=Sort1D(nums, canon) %CM: Rewritten on 22/03/03
            N=length(canon);
            kept=false(1,N);
        ...
        function idxs=IndexOf(v1, v2) %CM: Rewritten on 22/03/04
            N=length(v1);
            idxs=[]; idxs(N) = 0;
            nFound=0;
        ...
        function out=Trim(in, less) %CM: Rewritten on 22/03/04
            if nargin<2
                less=1;
            end
        ...
        function matrix=GetMatrixAsProp(props, name) %CM: Rewritten on 22/03/04
            matrix=[];
            A=props.getAll(name);
            R=length(A);
        ...
        function [out,ok]=CheckBounds(a, N)  %CM: Rewritten on 22/03/04
            inBounds = (a >= 1) & (a <= N);
            ok = all(inBounds);
        ...
        function counts=CountMinEdge(matrix,mins)  %CM: Rewritten on 22/03/04
            if size(mins, 2) ==1
                mins = mins';
            end
        ...
        function edgeRows=FindMinEdge(matrix, mins) %CM: Rewritten on 22/03/04
            if size(mins, 2) ==1
                mins = mins';
            end
...
%                    disp('hmm');
        ...
        function [counts, descendingOrder] = HistCounts(x, labels)
            if nargin<2
                labels=unique(x);
            end
            if isempty(labels)
        ...
        function [dists, idxs]=PercentCloses(clump, percent)
            if nargin<2
                percent=.8;
            end
