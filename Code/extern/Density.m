...
classdef Density < handle
    ...
    methods
         ...
         function [roi, strXy]=getROI(this, ax, callBack, tolerance, clusterIds)
             if nargin<5
                 cp = get(ax,'CurrentPoint');
                 clusterIds=this.getCluster(cp(1,1),cp(1,2));
                 if nargin<4
                     tolerance=1;
                 end
             end
         ...
         function drawBorders(this, ax, clr)
            wasHeld=ishold(ax);
            if ~wasHeld
                hold(ax, 'on');
            end
            ...
            Hs=zeros(1,N_);
            for i=1:N_
                Hs(i)=gridEdge(this, true, i, clr, ax, .8, '.', '-', .5);
