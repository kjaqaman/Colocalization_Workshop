...
for i = 1:nChan
    ...
    disp(outFilePaths{1,ch});
        ...
        %Run Detection    
    [maskBlobs,labels,maskBlobsVis,blobProperties,meanBlobProperties] = segmentBlobs_locmax(I,...
    thresholdMethod{ch},methodValue(ch),filterNoise(ch),filterBackground(ch),minSize(ch),locMax(ch),plotRes(ch),mask); 
        
    save(fullfile(saveResults(ch).dir,saveResults(ch).filename),'maskBlobs','labels','maskBlobsVis','blobProperties','meanBlobProperties');
end
