function [maskBlobs,labels,maskBlobsVis,blobProperties, meanBlobProperties] = segmentBlobs_locmax(image,...
...
blobProperties = regionprops(labels,image,'Area','Eccentricity','Circularity','MeanIntensity','PixelValues');

fld = fieldnames(blobProperties);
for i = 1:length(fld)
    if ~strcmp(fld{i},'PixelValues')
        tmp = vertcat(blobProperties.(fld{i}));
        tmp(~isfinite(tmp)) = nan;
        tmp = mean(tmp,'omitnan');
        meanBlobProperties.(fld{i}) = tmp;
    end
end
idx = find([blobProperties.Area] > minSize);
