function plotSaveConditionalColoc(PtOrBlob, plotting, saveRes, pathToSave, guiFlag, varargin)
%PLOTSAVECONDITIONALCOLOC plots and/or saves 3-color conditional colocalization results
%
...
%ignore the prompt asking for what to save.
%
...
if nargin < 1
    error('Specify type of Conditional Colocalization to be plotted. Enter 1 for plotting Pt2Pt2Pt, 2 for Pt2Pt2Blob, or 3 for General.')
    
elseif PtOrBlob ~= 1 && PtOrBlob ~= 2 && PtOrBlob ~=3
    error('Invalid enntry. Enter 1 for plotting Pt2Pt2Pt, 2 for PtPt2Blob, or 3 for General.')
...
if nargin < 3 || isempty(saveRes) || saveRes == 0
    ...
    % Added to omit console prompt asking what to save when using the function
    % in the GUI. Assumes that user wants to save all.(Mamerto Cruz, July 2022)
elseif saveRes == 1
    %whatToSave = 4;
    if guiFlag == 1
        whatToSave = 4;
        ...
        whatToSave = input(['\n' 'Do you want to save the figures, the compiled data values, the number of objects per cell'...
            ...
            '\n' 'Enter 1 for figures, 2 for compiled data values, 3 for number of objects per cell, 4 for all'...
            '\n' '\n' 'Enter number ']);
    end
end
...
%% Check MovieList input

%number of movie lists
numMLs = length(varargin);

%number of movies per MovieList
...
for ml = 1:numMLs
    ...
    if isa(varargin{ml},'MovieList')
        ...
        error('All varargin inputs must be MovieLists')
    end
end
...
for ml = 1:numMLs
    ...
    for mv = 1:moviesPerML(ml)
        ...
        %number of objects
        %KJ 240821: Here too sort in ASCII order, for the same reason
        if PtOrBlob ~= 3
            tempN = load(currentDirN);
            tempN = orderfields(tempN);
        ...
        for f = 1:numCategories
            ...
            elseif ~isstruct(tempC.(fieldNamesC{f})) && PtOrBlob == 3
                ...
                numCategories = numCategories - 1; %KJ 240821 note: This handles case of Pt2Pt2Blob where one additional variable is stored in the output file, but it is not of interest here
...
for f = 1:numCategories
    ...
    %% Assemble results for current category (f)
    ...
    %% Plot results for current category (f)
    ...
    if plotting
        ...
        labelX = {'p(TwR)','p(TwR|TwC)','p(TwR|TnC)','p^{rs}(Tw(RwC))','p^{rs}(Tw(RnC))'};
        pLabelX = {'p(TwC)' 'p(RwC)'};
        ...
        for bx=1:2
            ...
            if bx == 1
                ...
                annotation('textbox',[0.2 0.5 0.3 0.3],'string',{'Blue = Data',...
                    'Black = nullTR','Green = randC'},'fitboxtotext','on')
            ...
            if whatToSave == 1 || whatToSave == 4
                ...
                if isempty(pathToSave)%ask for path if it wasn't input
                    if bx == 1
                        [file,pathToSave] = uiputfile('*.fig','Choose where to save figure', ['condProbabilities' fieldNamesC{f}(15:end) '.fig']);
                        
                    elseif bx == 2
                        [file,pathToSave] = uiputfile('*.fig','Choose where to save figure', ['pTarRefWithCond' fieldNamesC{f}(15:end) '.fig']);
                    ...
                    if bx == 1
                        savefig([pathToSave '/condProbabilities' fieldNamesC{f}(15:end) '.fig'])
                    elseif bx == 2
                        savefig([pathToSave '/pTarRefWithCond' fieldNamesC{f}(15:end) '.fig'])
                    end
                end
    ...
    %% Save colocalization values for current category
    ...
    if whatToSave == 2 || whatToSave == 4
        ...
        if isempty(pathToSave)%ask where to save if no path was input
            ...
            if pathToSave == 0 %if no path entered on dialog box show warning and continue
    ...
    %% Save number of objects information for current category
...
if whatToSave && PtOrBlob == 3
    ...
    save(fullfile(pathSaveInfo,'compiledDataInfo.mat'),'dataCompiledFrom',...
